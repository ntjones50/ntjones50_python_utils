...
def set_df_series_dtypes(in_df: df, in_dtypes_dict: dict) -> df:
    ...
    int64, datetime64[us]. The function returns a new Pandas dataframe with the updated column types.

    Inputs:

        in_df (Pandas DataFrame) =      The source Pandas Dataframe from which columns will be updated by the
                                        the function. Each column name needs to be a string.
        ...
        in_dtypes_dict (Dictionary) =   An input dictionary where each key is a string and corresponds to a 
                                        ...
                                        boolean, float64, int64, datetime64[us]. Not every column in in_df
    ...
    allowable_types = ['object', 'string', 'boolean', 'float64', 'int64', 'datetime64[us]']
    for t_key in wrk_dict.keys():
    ...
    # Update wrk_df
    for t_key in wrk_dict.keys():
        if not wrk_dict[t_key] == 'datetime64[us]':
